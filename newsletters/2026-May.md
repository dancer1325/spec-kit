...
* milestone 100s
  ...
  * **top-100 GitHub project**

| **Community & Content** | **SDD Ecosystem & Next** |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| --- | --- |
...
* **v0.8.4–v0.8.7** (May 1–7)
  * the MOST-requested feature of the year
    * **multi-install support -- for -- concurrent AI agent integrations** (#2389)
    * multiple agents | 1! project
    * **constitution governance | `/speckit.implement`** (#2460)
    * ensuring the implement phase now loads `constitution.md` to enforce governance during code generation
    * **authentication provider registry** (#2393)
    * The **Lingma agent** joined the integration roster
    * TODO:Security hardening included pinning all remaining GitHub Actions to immutable SHAs (#2441) and URL scheme validation to prevent SSRF-style bugs (#2449)
    * Seven new community extensions and six new governance-themed presets landed
...
* **`__init__.py` decomposition series**
  * PRs 1–4 of 8
* This systematic extraction moved `_console.py`, `_assets.py`, `_utils.py`, `_version.py`, and the `commands/` package out of the monolithic init module, improving maintainability and contributor onboarding
* The **ExtensionCatalog was migrated to the shared catalog stack base** (#2437), reducing duplicated catalog handling across extension, preset, and integration catalogs
...
* [MarkTechPost comparison](https://www.marktechpost.com/2026/05/08/9-best-ai-tools-for-spec-driven-development-in-2026-kiro-bmad-gsd-and-more-compare/)
  * f nine SDD tools called Spec Kit "the most community-adopted open-source option," while positioning competitors along distinct axes
    * **Kiro** (integrated IDE with EARS-based specs and agent hooks)
    * **BMAD-METHOD** (~48K stars, 12+ specialized agents)
    * **GSD** (~64K stars, lean meta-prompting)
    * **Augment Code** (context engine for 400K+ files, not a spec authoring tool)
    * **OpenSpec** (~52K stars, change accountability and audit trails)
    * **Tessl** (spec registry with 10K+ library specs)

With 107K stars at month's end, Spec Kit is the **only spec-driven development tool in the top 100 most-starred repositories on GitHub** 
none of the competitors above are close to the 100K threshold
The broader top-100 list includes AI-adjacent projects like agentic skills frameworks
(obra/superpowers at 212K, anthropics/skills at 143K), agent harness tools, and LLM inference engines, 
but Spec Kit is the only one built around a spec-first development workflow. [\[github.com\]](https://github.com/search?q=stars%3A%3E100000&type=repositories&s=stars&o=desc)
